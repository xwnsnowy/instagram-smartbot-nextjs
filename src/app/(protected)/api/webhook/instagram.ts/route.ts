import { findAutomation } from "@/actions/automations";
import { createChatHistory, getChatHistory, getKeywordAutomation, getKeywordPost, matchKeyword, trackResponses } from "@/actions/webhook";
...
export async function POST(req: NextRequest) {
  ...
  try {
          ...
          if (automation.listener) {
            ...
            if (
              ...
              if (smart_ai_message.choices[0].message.content) {
                ...
                const token = automation.User?.integrations[0]?.token; if (!token) {
                  throw new Error("Token not found");
                }
                const direct_message = await sendPrivateMessage(
                  webhook_payload.entry[0].id,
    ...
    console.log('🔴 server side Error in webhook ', error)
    return NextResponse.json(
      {
        message: 'No automation set',
      },
